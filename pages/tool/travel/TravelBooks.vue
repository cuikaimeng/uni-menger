		...
		<div class="waterfall-container" ref="containerRef" :style="{ height: containerHeight + 'px' }">
			<div v-for="(item, index) in list" :key="index" class="waterfall-item" :ref="(el) => setItemRef(el, index)"
				...
				<div class="card-content">
					<TravelBook :book-data="item" :book-index="index" @load="handleImgLoad"></TravelBook>
				</div>
			</div>
...
import TravelBook from '@/pages/tool/travel/TravelBook.vue'
...
.waterfall-item {
	position: absolute; /* 必须显式写在这里或者由 JS 注入 */
	left: 0;
	top: 0;
	background: #fff;
	border-radius: 8px;
	overflow: hidden;
	opacity: 0;
	transition: opacity 0.3s ease, transform 0.3s ease; /* 建议加上 transform 动画 */
}
